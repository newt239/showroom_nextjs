...
const Player: NextPage = () => {
  ...
  return (
    <Flex style={{ flexDirection: 'column', gap: '.5rem' }}>
      <Button onClick={downBottom} colorScheme='blue' size='sm' disabled={!game.start || game.end}>
        <ArrowUpIcon />
      </Button>
      <Button onClick={blockDown} colorScheme='blue' size='sm' disabled={!game.start || game.end}>
        <ArrowDownIcon />
      </Button>
      <Button onClick={blockLeft} colorScheme='blue' size='sm' disabled={!game.start || game.end}>
        <ArrowBackIcon />
      </Button>
      <Button onClick={blockRight} colorScheme='blue' size='sm' disabled={!game.start || game.end}>
        <ArrowForwardIcon />
      </Button>
      <Button onClick={rotate} colorScheme='blue' size='sm' disabled={!game.start || game.end}>
        <RepeatIcon />
      </Button>
      <Button onClick={startGame} colorScheme='blue' disabled={game.start}>
        start
      </Button>
      <Button onClick={resetGame} colorScheme='red' disabled={!game.start || !game.end}>
        reset
      </Button>
    </Flex>
  )
