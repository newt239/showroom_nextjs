  ...
  const reducer = (state: GameStateType, action: ActionType) => {
    ...
    if (typeof action.payload === "number") {
      if (action.type === "x") {
        return {
          ...state,
          current: { ...state.current, x: state.current.x + action.payload },
        };
      } else if (action.type === "y") {
        return {
          ...state,
          current: { ...state.current, y: state.current.y + action.payload },
        };
      }
    } else if (
      action.type === "state" &&
      ...
      const savedBoard = state.board.map((row, i) => {
        return row.map((cell, j) => {
          if (cell !== 0) {
            ...
            return state.current.state[i - state.current.y][
              j - state.current.x
            ];
          } else {
            return 0;
      ...
      return {
        ...
        current: {
          type: state.current.next,
          state: tetromino[state.current.next],
          x: 0,
          y: state.current.next === 1 ? 0 : -1,
